...
1. **#121의 뿌리는 `:nowait`** — 핸드셰이크는 6월 rustls facade로 이미 고쳐졌다.
   ...
   폴백 → 400 → ELPA 빈 archive. 이슈 본문의 "GnuTLS error"와 현재 실체가 다르다.
   재현: `./bin/neomacs.sh --probe tls`
2. **org 표 안의 링크가 열 폭을 부풀린다** (신규 발견, upstream 미보고).
   GNU는 `org-link-display-format` 폭(4칸), Neomacs는 원시 대괄호 29칸으로 열을 잡음.
   elisp 레벨 입력은 전부 동일 → `org-table-align` 내부 폭 계산 갈라짐.
   **실사용 주의**: 링크 있는 표를 건드리면 파일이 부풀어 다시 써진다.
...
**미착수 (다음 회차 이후)**:
- treesit 문법 미설치 (다운로드에 TLS 필요). 시스템에서 넣으면 하이라이트 붙음.
- 성능 비교 — 8데몬 35분 export 벤치마크 대조는 아직. 7월 concurrent GC
  (SATB, arena pages, TSan 게이트)로 비교 근거는 생겼다.

리서치 노트: Denote `20260209T123532` 회차 `2026-07-19`.
