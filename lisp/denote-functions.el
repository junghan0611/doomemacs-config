...
(defun my/denote-org-dblock-insert-meta-links ()
  ...
  (let* ((topics (mapcar (lambda (file)
    ...
    (org-create-dblock (list :name "denote-links"
                             ...
                             :not-regexp nil
                             :excluded-dirs-regexp
                             "\\(bib\\|notes\\|office\\|elisp\\|botlog\\|llmlog\\|docs\\|posts\\|md\\|journal\\|transcript\\|dict\\|private\\|ekg\\)"
                             :sort-by-component nil
                             :reverse-sort nil
                             :id-only nil
                             :include-date t))
...
With prefix ARG, prompt for a date via calendar."
  ...
  (let* ((date (my/org--read-date-or-today arg))
         ...
         (regexp (concat ymd "T*")))
    (insert
     (format "#+BEGIN: denote-links :regexp \"%s\" :not-regexp nil :excluded-dirs-regexp \"\\\\(journal\\\\|office\\\\|transcript\\\\|archive\\\\|md\\\\|dict\\\\|posts\\\\|private\\\\|ekg\\\\)\" :sort-by-component nil :reverse-sort nil :id-only nil :include-date t\n#+END:\n"
             regexp))))
...
With prefix ARG, prompt for a date via calendar."
  ...
  (let* ((date (my/org--read-date-or-today arg))
         ...
         (ymd (format-time-string "%Y-%m-%d" time)))
    (insert
     (format "#+BEGIN: denote-lastmod :from \"%s\" :to \"%s\" :excluded-dirs-regexp \"\\\\(journal\\\\|office\\\\|archive\\\\|transcript\\\\|md\\\\|dict\\\\|posts\\\\|private\\\\|ekg\\\\)\"\n#+END:\n"
             ymd ymd))))
