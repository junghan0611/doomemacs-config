#!/usr/bin/env bash
# Emacs 31 channel launcher
# 시스템 stable과 공존 — server-name "doom-unstable"로 분리.
# flake output 이름은 호환상 `emacs-unstable`이지만 실제 패키지는
# nixos-unstable의 released Emacs 31 GTK3 build다.
#
# Usage:
...
echo "[emacs-unstable] resolving Emacs 31 store path (nix build, may take a while on cache miss)..." >&2
