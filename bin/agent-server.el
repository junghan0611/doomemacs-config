...
;;;; Headless Buffer Safety
;; CRITICAL: 이 데몬에는 터미널이 없다. `yes-or-no-p' / `y-or-n-p' 가 뜨면
;; 영원히 멈춘다(bbot 입장에선 emacsclient 가 hang — issue #9).
;;
;; 노트가 out-of-band 로 수정된 채(에이전트 Write 도구, 실패한 denote rename,
;; 또는 인간 Doom) 그 파일의 버퍼가 데몬에 남아 있으면 두 가지로 터진다:
;;
;;   1. `find-file-noselect' 가 stale 버퍼를 돌려주고, 이어지는
;;      `insert' / `save-buffer' 가 `ask-user-about-supersession-threat'
;;      ("file changed on disk; really edit?")를 띄움 → hang.
;;   2. 프롬프트가 없더라도 stale 내용에 append 후 저장하면 디스크 변경을 덮어씀.
;;
;; 해결: 항상 디스크에서 새로 연다(`agent-server--find-file-fresh'). 그리고
;; 최후의 가드로, supersession 프롬프트를 signal 로 바꿔 빠져나간 케이스가
;; hang 대신 진단 문자열을 반환하게 한다.

(defun agent-server--discard-visiting-buffer (file)
  "FILE 을 visiting 하는 버퍼를 죽여 다음 open 이 현재 디스크 상태를 읽게 한다.
저장 안 된 편집은 버린다(에이전트 작업에선 디스크가 SSOT). modified/kill
프롬프트도 함께 눌러 헤드리스 데몬이 막히지 않게 한다.  `find-buffer-visiting'
은 truename 으로 매칭하므로 ~/org ↔ ~/sync/org 심볼릭도 처리된다."
...
(defun agent-server--find-file-fresh (file)
  "`find-file-noselect' 와 같되 FILE 을 항상 현재 디스크에서 새로 읽는다.
근거는 `agent-server--discard-visiting-buffer' 참고 (issue #9)."
  (agent-server--discard-visiting-buffer file)
  (find-file-noselect file))

;; 최후 가드: supersession 프롬프트에서 막히지 말고 signal 하여, 감싸는
;; `condition-case' 가 "ERROR: ..." 문자열을 에이전트에게 돌려주게 한다.
(advice-add 'ask-user-about-supersession-threat :override
            (lambda (filename)
              (signal 'file-supersession
                      (list "stale agent buffer 위에서 디스크가 변경됨 — 다시 열고 재시도"
                            filename))))
...
불일치 시 결과에 WARN 포함."
  ...
  (if (not (file-exists-p file))
      (format "ERROR: File not found: %s" file)
    ;; 디스크에서 새로 읽기: stale 버퍼를 버려 denote 내부 save 가
    ;; hang 유발 supersession 프롬프트를 띄우지 못하게 한다. (issue #9)
    (agent-server--discard-visiting-buffer file)
    (let ((denote-rename-confirmations nil)
          (denote-save-buffers t)
          (denote-kill-buffers 'on-rename)
          ;; `denote-rename-file-and-buffer' → `vc-rename-file' 는 현재
          ;; `default-directory' 를 cwd 로 `git mv' 를 돌리고 인자를
          ;; `file-relative-name' 로 만든다.  이 데몬에선 그 cwd 가
          ;; emacsclient 가 eval 한 자리 — ~/org repo 밖 — 라서 git 이
          ;; `../org/botlog/…' 상대경로 + .git 없는 cwd 를 받아 "status 128"
...
결과를 (renamed skipped errors) 카운트로 반환."
  ...
  (let ((files (directory-files-recursively directory "\\.org$"))
        ...
        (denote-save-buffers t)
        (denote-kill-buffers 'on-rename)
        ;; git cwd 를 repo 안으로 고정 — `vc-rename-file' → `git mv' 가 repo 를
        ;; 찾게 한다 (`agent-denote-rename-by-front-matter' 주석 참고). (issue #9)
